...
\subsection{Ideal Path Data Genration:}
To find the ideal constants of potential field , we first found the ideal path by using BFS and Dijkstra. This ideal paths took a considerable amount of time to form, but this a one-time investment for training. Once, the ideal path for all the 1,00,000 is generated is stored in the form of a list of coordinates for further use. These ideal paths were then made use of in the neural networks. An example is shown in the figure below (Fig 3).
 \vspace{5mm}
